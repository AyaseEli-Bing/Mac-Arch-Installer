...
- **`vm-check.sh` 时钟检查**：以外部时间源实测本机 UTC 偏差，≤60s 判正常、≤600s 判警告、
  超过判失败，超阈值时打印校正命令但仍遵守本脚本的只读约定，不代为执行。
  基准取 **HTTP**（而非 HTTPS）镜像站响应的 `Date` 头 —— 时钟偏到证书有效期之外时
  HTTPS 自身就握手失败，拿它校时钟会陷入循环依赖，而那恰恰是最需要检查的时刻。
- **`diagnose.sh` 宿主机↔虚拟机时钟偏差**：直接以宿主机时间为基准，不依赖外网。
  前后各取一次宿主机时间并以中点作基准，避免 SSH 建连耗时造成误判。

> 两项都刻意不采信 `timedatectl` 的 `System clock synchronized` 字段：虚拟机配置为关窗即
> 挂起，挂起期间客户机时钟停走，恢复后停留在挂起那一刻（实测偏差 30 分钟），
> 而该字段仍报 `yes`。验证方式是把虚拟机时钟人为拨慢 45 分钟，两项均正确报 `FAIL`
> 并给出 `sudo hwclock --hctosys`。
