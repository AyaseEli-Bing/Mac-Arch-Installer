...
.
├── arch-install/
│   ├── install.sh            # [VM 内]   自动安装：镜像探活 → 分区 → 装包 → 配置 → 引导
...
├── 自检功能使用教程.md                       # 三个自检工具的作用、检查项与结果解读
...
分 4 组检查：宿主机工具、安装介质、虚拟机状态、分发服务。
全部 `[PASS]` 后再继续——各项的检查内容、结果含义与失败处理，
见 **[自检功能使用教程.md](自检功能使用教程.md)**。
...
| **[自检功能使用教程.md](自检功能使用教程.md)** | 三个自检工具（环境自检 / 健康巡检 / 故障诊断）的作用、检查项、结果含义与行动指引 |
